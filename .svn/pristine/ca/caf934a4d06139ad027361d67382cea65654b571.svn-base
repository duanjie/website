...
//	$(".ui-datepicker-month, .styled, .dataTables_length select").uniform({ radioClass: 'choice' });
    ...
    //====Checkbox select all========//
function  initCheckBoxSelectAll(checkAllRow,checkRow){	
	$("input[name='"+checkAllRow+"']").change(function() {
	   if(!$(this).attr("checked")) {
    	   $(this).parent().removeClass("checked");
    }else if($(this).attr("checked")){
    	   $(this).parent().addClass("checked");
    }  
	   var checkAllRowChecked =	$(this).attr("checked");
		$("input[name='"+checkRow+"']").each(function(){
		  if(checkAllRowChecked){
	        	$(this).parent().addClass("checked");
...
//=========Form Submit==========//
function initForm(formName,successUrl){
	var ajax_option={
		dataType:"json",
		success:function(data){ 
			$.message(data.type, data.content);
			if(data.type=="success"){
				loadContent(successUrl);
			}
		},
	...
	$("form[name="+formName+"]").submit(function() {
		if(typeof(CKEDITOR.instances.content) != "undefined"){
			...
			$("#content").val(encodeURIComponent(CKEDITOR.instances.content.getData()));
		}else{
			$("#content").val(encodeURIComponent($("#content").val()));
		}
		var validate =  $("form[name="+formName+"]").validationEngine("validate");
	 	if(validate){
			$(this).ajaxSubmit(ajax_option); 
		}
 	    return false;
 	   });
	$("form[name="+formName+"]").validationEngine({promptPosition : "topRight:-122,-5"});
}


function initSearchForm(formName){
	var ajax_option={
...
/**
 * @param url 修改页面路径
 * @param checkBoxName 选择框Name
 */
function onEdit(url,checkBoxName){
	var $checkRow = $("input[name='"+checkBoxName+"']:checked");
	var length = $checkRow.length;
	if(length != 1){
		$.message("error", "请选择一个选项");
		return;
	}
	var id = $checkRow[0].value;
 ...
 * @param checkBoxName  选择框Name
 */
function onDelete(url,refreshUrl,checkBoxName){
	var $checkRow = $("input[name='"+checkBoxName+"']:checked");
	var length = $checkRow.length;
	if(length == 0){
...
function onDeleteById(url,refreshUrl,id){
	bootbox.confirm('确认删除？',function(rs){
		  if(rs){
			 $.ajax({   
		         type: 'POST',   
...
/**
 * @param url 删除后台路径
 * @param refreshUrl  操作完成后进入的页面
 */
function onDeleteAll(url,refreshUrl){
	bootbox.confirm('确认删除全部？',function(rs){
		  if(rs){
				 $.ajax({   
			         type: 'POST',   
			         url:url,  
