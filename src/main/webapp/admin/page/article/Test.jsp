				  ...
				  <div class="crumbs">
                    ...
                    <div class="table-overflow">
                        <table  class="table table-striped table-bordered table-checks media-table">
                            <thead>
                                <tr>
                                    <th width="10%"><div class="checker" id="uniform-undefined1"><span><input type="checkbox"  name="checkAllRow" class="styled" style="opacity: 0;" ></span></div></th>
                                    <th width="10%">id字段</th>
                                    <th width="10%">名字</th>
                                    <th width="10%">创建时间</th>
                                    <th width="10%">修改时间</th>
                            ...
                            <tbody   class="table table-striped table-bordered table-checks media-table">
                              <c:forEach items="${page1.content}" var="article" varStatus="status">
	                               <tr id="cls" <c:if test="${status.index % 2 != 0}">class="info" </c:if>>
	                                   <td><div class="checker" id="uniform-undefined1"><span ><input type="checkbox" name="checkRow"  class="styled" style="opacity: 0;"/></span></div></td>
	                                   <td id="ids" ><c:out value="${test.menuId}"/></td>
	                                   <td><c:out value="${test.name}"/></td>
	                                   <td><c:out value="${test.create_date}"/></td>
	                                   <td><c:out value="${test.modify_date}"/></td>
	                                   <td><c:out value="${test.menu}"/></td>
	                                   <td><c:out value="${test.instruction}"/></td>
	                               </tr>
                              </c:forEach>
                            </tbody>
                        </table>
                    ...
                    <div class="table-footer">
                        <div class="pagination">
                            <ul>
                                <li><a href="javascript:loadContent('admin/article/list1.jhtml?pageNumber=${page.pageNumber-1}')">上一页</a></li>
                               	<c:forEach var="pageNumber" begin="1" end="${page.totalPages}" step="1"> 
	                                <li <c:if test="${pageNumber == page.pageNumber}">class="active"</c:if> >
	                               	 <a href="javascript:loadContent('admin/article/list1.jhtml?pageNumber=${pageNumber}')">${pageNumber}</a>
	                                </li>
                               </c:forEach>
                                <li><a href="javascript:loadContent('admin/article/list1.jhtml?pageNumber=${page.pageNumber+1}')">下一页</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
