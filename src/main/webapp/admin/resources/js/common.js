	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: mylsh.base + "/admin/file/browser.jhtml",
				uploadUrl: mylsh.base + "/admin/file/upload.jhtml",
				callback: null
			};
			$.extend(settings, options);
			...
			return this.each(function() {
				...
				$browserButton.click(function() {
					...
					var path = $browserList.find("img").next().text();
					if (settings.isUpload) {
						...
						$browserForm = $('<form action="' + settings.uploadUrl + '" method="post" encType="multipart/form-data" target="' + browserFrameId + '"><input type="hidden" name="token" value="' + token + '" \/><input type="hidden" name="path" value="' + path + '" \/><input type="hidden" name="fileType" value="' + settings.type + '" \/><\/form>').appendTo($browserBar);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = mylsh.base + "/admin/resources/img/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = mylsh.base + "/admin/resources/img/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
						...
						$browserList.find("img").bind("click", function() {
							...
							if (isDirectory == "true") {
								...
								if (settings.input != null) {
									...
									var $thumbnail = $browserButton.prevUntil()().attr(".thumbnail");
